...
public class AndroidPlatformSupport extends PlatformSupport {
	...
	public void updateDisplaySize(){
		if (UPDSettings.landscape() != null) {
			AndroidLauncher.instance.setRequestedOrientation( UPDSettings.landscape() ?
		...
		if (fullscreen && UPDSettings.landscape() != null
				&& (Game.dispWidth >= Game.dispHeight) != UPDSettings.landscape()){
		...
		if (Game.dispWidth < renderWidth*2 || Game.dispHeight < renderHeight*2)
			UPDSettings.put( UPDSettings.KEY_POWER_SAVER, true );
		
		if (UPDSettings.powerSaver() && fullscreen){

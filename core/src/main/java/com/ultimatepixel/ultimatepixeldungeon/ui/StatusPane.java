...
public class StatusPane extends Component {
	...
	public StatusPane( boolean large ){
		...
		if (large){
			...
			expText.hardlight( Xml.getColor("shalimar_cream") );
			expText.alpha(0.6f);
			add(expText);
		}

		level = new BitmapText( PixelScene.pixelFont);
		level.hardlight( Xml.getColor("shalimar_cream") );
		add( level );
		...
		counter.color( Xml.getColor("grey"), true );
	...
	public void update() {
		...
		if (!Dungeon.hero.isAlive()) {
			avatar.tint(Xml.getColor("pure_black"), 0.5f);

...
import com.ultimatepixel.ultimatepixeldungeon.scenes.CellSelector;
...
import com.ultimatepixel.ultimatepixeldungeon.windows.WndQuickBag;
...
public class Toolbar extends Component {
	...
	protected void createChildren() {
		...
		add(new Button(){
			...
			protected void onClick() {
				...
				if (Dungeon.hero.ready && !GameScene.cancel()) {
					...
					if (ControllerHandler.controllerActive){
						...
						info += Messages.get(WndKeyBindings.class, UPDAction.LEFT_CLICK.name()) + ": " + Messages.get(Toolbar.class, "quickslot_select") + "\n";
						info += Messages.get(WndKeyBindings.class, UPDAction.RIGHT_CLICK.name()) + ": " + Messages.get(Toolbar.class, "quickslot_assign") + "\n";
			...
			public GameAction keyAction() {
				if (btnWait.active) return UPDAction.QUICKSLOT_SELECTOR;
				else				return null;
			}
		});
		...
		add(btnWait = new Tool(24, 0, 20, 26) {
			...
			@Override
			public GameAction keyAction() {
				return UPDAction.WAIT;
			}

			@Override
			public GameAction secondaryTooltipAction() {
				return UPDAction.WAIT_OR_PICKUP;
			}
		...
		add(new Button(){
			...
			@Override
			public GameAction keyAction() {
				if (btnWait.active) return UPDAction.REST;
				else				return null;
			}
		});
		...
		add(new Button(){
			...
			@Override
			public GameAction keyAction() {
				if (btnWait.active) return UPDAction.WAIT_OR_PICKUP;
				else				return null;
			}
		});
		...
		add(btnInventory = new Tool(0, 0, 24, 26) {
			...
			protected void onClick() {
				if (Dungeon.hero.ready || !Dungeon.hero.isAlive()) {
					if (UPDSettings.interfaceSize() == 2) {
			...
			@Override
			public GameAction keyAction() {
				return UPDAction.INVENTORY;
			}

			@Override
			public GameAction secondaryTooltipAction() {
				return UPDAction.INVENTORY_SELECTOR;
			}
			...
			protected void createChildren() {
				...
				arrow.visible = UPDSettings.interfaceSize() == 2;
				arrow.tint(Xml.getColor("black_magic"), 1f);
				add(arrow);
		...
		add(new Button(){
			...
			protected void onClick() {
				if (Dungeon.hero.ready && !GameScene.cancel()) {
					...
					if (ControllerHandler.controllerActive){
						...
						info += Messages.get(WndKeyBindings.class, UPDAction.LEFT_CLICK.name()) + ": " + Messages.get(Toolbar.class, "container_select") + "\n";
					...
					GameScene.show(new RadialMenu(Messages.get(Toolbar.class, "container_prompt"), info, names, images){
						...
						public void onSelect(int idx, boolean alt) {
							...
							if (ControllerHandler.controllerActive){
								...
								info += Messages.get(WndKeyBindings.class, UPDAction.LEFT_CLICK.name()) + ": " + Messages.get(Toolbar.class, "item_select") + "\n";
								info += Messages.get(WndKeyBindings.class, UPDAction.RIGHT_CLICK.name()) + ": " + Messages.get(Toolbar.class, "item_use") + "\n";
								info += KeyBindings.getKeyName(KeyBindings.getFirstKeyForAction(GameAction.BACK, false)) + ": " + Messages.get(Toolbar.class, "item_cancel");
							}
			...
			@Override
			public GameAction keyAction() {
				if (btnWait.active) return UPDAction.INVENTORY_SELECTOR;
				else				return null;
			}
		});
	...
	protected void layout() {
		...
		int startingSlot;
		if (UPDSettings.quickSwapper() && quickslotsToShow < 6){
			quickslotsToShow = 3;
		...
		for(int i = startingSlot; i <= endingSlot; i++) {
			if (i == startingSlot && !UPDSettings.flipToolbar() ||
				i == endingSlot && UPDSettings.flipToolbar()){
				btnQuick[i].border(0, 2);
				btnQuick[i].frame(106, 0, 19, 24);
			} else if (i == startingSlot && UPDSettings.flipToolbar() ||
					i == endingSlot && !UPDSettings.flipToolbar()){
				btnQuick[i].border(2, 1);
				btnQuick[i].frame(86, 0, 20, 24);
			} else {
		...
		float shift = 0;
		switch(Mode.valueOf(UPDSettings.toolbarMode())){
			case SPLIT:
				btnWait.setPos(x, y);
	...
	private static class Tool extends Button {
		...
		private static final int BGCOLOR = Xml.getColor("gunsmoke");

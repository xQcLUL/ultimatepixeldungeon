		...
		keyBlocker = new Signal.Listener<KeyEvent>(){
			...
			public boolean onSignal(KeyEvent keyEvent) {
				if (keyEvent.pressed && isSelecting() && InventoryPane.this.visible
						&& KeyBindings.getActionForKey(keyEvent) != UPDAction.BAG_1
						&& KeyBindings.getActionForKey(keyEvent) != UPDAction.BAG_2
						&& KeyBindings.getActionForKey(keyEvent) != UPDAction.BAG_3
						&& KeyBindings.getActionForKey(keyEvent) != UPDAction.BAG_4
						&& KeyBindings.getActionForKey(keyEvent) != UPDAction.BAG_5){
					//any windows opened as a consequence of this should be centered on the inventory
					GameScene.centerNextWndOnInvPane();
					selector.onSelect(null);
					selector = null;

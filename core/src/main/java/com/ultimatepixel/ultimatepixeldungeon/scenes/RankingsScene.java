...
import com.ultimatepixel.ultimatepixeldungeon.windows.WndDailies;
import com.ultimatepixel.ultimatepixeldungeon.windows.WndError;
import com.ultimatepixel.ultimatepixeldungeon.windows.WndRanking;
	...
	public void create() {
		...
		if (Rankings.INSTANCE.records.size() > 0) {
			...
			if (Rankings.INSTANCE.totalNumber >= Rankings.TABLE_SIZE) {
				...
				label.setHightlighting(true, Window.UPX_COLOR);
		...
		if (Rankings.INSTANCE.latestDaily != null) {
			IconButton btnDailies = new IconButton(Icons.CALENDAR.get()) {
				...
				protected void onClick() {
					UltimatePixelDungeon.scene().addToFront(new WndDailies());
				}

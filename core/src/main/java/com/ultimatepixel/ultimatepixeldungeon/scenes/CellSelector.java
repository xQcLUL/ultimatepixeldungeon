...
public class CellSelector extends ScrollArea {
	...
	private float zoom( float value ) {
		...
		UPDSettings.zoom((int) (value - PixelScene.defaultZoom));
		camera.zoom( value );
	...
	//used for movement
	private GameAction heldAction1 = UPDAction.NONE;
	private GameAction heldAction2 = UPDAction.NONE;
	//not used for movement, but helpful if the player holds 3 keys briefly
	private GameAction heldAction3 = UPDAction.NONE;

	private float heldDelay = 0f;
	private boolean delayingForRelease = false;

	private static float initialDelay(){
		switch (UPDSettings.movementHoldSensitivity()){
			case 0:
	...
	private Signal.Listener<KeyEvent> keyListener = new Signal.Listener<KeyEvent>() {
		...
		public boolean onSignal(KeyEvent event) {
			...
			if (!event.pressed){
				...
				if (action == UPDAction.ZOOM_IN){
					zoom( camera.zoom+1 );
					mouseZoom = camera.zoom;
					return true;

				} else if (action == UPDAction.ZOOM_OUT){
					zoom( camera.zoom-1 );
					mouseZoom = camera.zoom;
					return true;
				}
				
				if (heldAction1 != UPDAction.NONE && heldAction1 == action) {
					heldAction1 = UPDAction.NONE;
					if (heldAction2 != UPDAction.NONE){
						heldAction1 = heldAction2;
						heldAction2 = UPDAction.NONE;
						if (heldAction3 != UPDAction.NONE){
							heldAction2 = heldAction3;
							heldAction3 = UPDAction.NONE;
						}
					}
				} else if (heldAction2 != UPDAction.NONE && heldAction2 == action){
					heldAction2 = UPDAction.NONE;
					if (heldAction3 != UPDAction.NONE){
						heldAction2 = heldAction3;
						heldAction3 = UPDAction.NONE;
					}
				} else if (heldAction3 != UPDAction.NONE && heldAction3 == action){
					heldAction3 = UPDAction.NONE;
				}
				...
				lastCellMoved = -1;
				if (heldAction1 == UPDAction.NONE){
					heldAction1 = action;
					heldDelay = initialDelay();
					delayingForRelease = false;
				} else if (heldAction2 == UPDAction.NONE){
					heldAction2 = action;
				} else {
					heldAction3 = action;
				}
	...
	private GameAction leftStickAction = UPDAction.NONE;
	...
	public void update() {
		...
		if (newLeftStick != leftStickAction){
			if (leftStickAction == UPDAction.NONE){
				heldDelay = initialDelay();
				Dungeon.hero.resting = false;
			} else if (newLeftStick == UPDAction.NONE && heldDelay > 0f){
				heldDelay = 0f;
		...
		if ((heldAction1 != UPDAction.NONE || leftStickAction != UPDAction.NONE) && Dungeon.hero.ready){
			processKeyHold();
	...
	private int directionFromAction(GameAction action){
		if (action == UPDAction.N)  return -Dungeon.level.width();
		if (action == UPDAction.NE) return +1-Dungeon.level.width();
		if (action == UPDAction.E)  return +1;
		if (action == UPDAction.SE) return +1+Dungeon.level.width();
		if (action == UPDAction.S)  return +Dungeon.level.width();
		if (action == UPDAction.SW) return -1+Dungeon.level.width();
		if (action == UPDAction.W)  return -1;
		if (action == UPDAction.NW) return -1-Dungeon.level.width();
		else                        return 0;
	}
	...
	public void resetKeyHold(){
		heldAction1 = heldAction2 = heldAction3 = UPDAction.NONE;
	}

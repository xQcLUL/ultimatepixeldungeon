...
public class AboutScene extends PixelScene {
	...
	public void create() {
		...
		//*** Ultimate Pixel Dungeon Credits ***

		CreditsBlock ulpd = new CreditsBlock(true, Window.UPX_COLOR,
				"Ultimate Pixel Dungeon",
				Icons.ULPD.get(),
				"Developed by: _Unknown_\nBased on Shattered Pixel Dungeon's open source",
				null,
				null);
		ulpd.setRect((Camera.main.width - colWidth)/2f, 6, 120, 0);
		content.add(ulpd);
		...
		CreditsBlock shpx = new CreditsBlock(true, Window.SHPX_COLOR,
				...
				shpxLink);
		if (landscape()){
			shpx.setRect((w - fullWidth)/2f - 6, ulpd.bottom() + 8, 120, 0);
		} else {
			shpx.setRect((w - fullWidth)/2f, ulpd.bottom() + 8, 120, 0);
		}
		content.add(shpx);
		...
		final int WATA_COLOR = Xml.getColor("cornflower_blue");
		...
		final int GDX_COLOR = Xml.getColor("cinnabar");
		CreditsBlock gdx = new CreditsBlock(true,
				GDX_COLOR,
				"libGDX",
				Icons.LIBGDX.get(),
				"UltimatePD is powered by _libGDX_!",
				"libGDX.com",
				"https://libGDX.com/");
		if (landscape()){
		...
		CreditsBlock transifex = new CreditsBlock(true,
				...
				null,
				null,
				"UltimatePD is community-translated via _Transifex_! Thank you so much to all of Shattered's volunteer translators!",
			...
			if (linkText != null && linkUrl != null){
				...
				linkButton = new PointerArea(0, 0, 0, 0){
					...
					protected void onClick( PointerEvent event ) {
						UltimatePixelDungeon.platform.openURI( linkUrl );
					}
				};
				add(linkButton);
			}

	...
	public static void showFlareForBonusDrop( Visual vis ){
		...
		switch (latestDropTier){
			default:
				break; //do nothing
			case 1:
				new Flare(6, 20).color(Xml.getColor("lime"), true).show(vis, 3f);
				break;
			case 2:
				new Flare(6, 24).color(Xml.getColor("deep_sky_blue"), true).show(vis, 3.33f);
				break;
			case 3:
				new Flare(6, 28).color(Xml.getColor("electric_purple"), true).show(vis, 3.67f);
				break;
			case 4:
				new Flare(6, 32).color(Xml.getColor("orange"), true).show(vis, 4f);
				break;
		}
		latestDropTier = 0;
	}

...
public class Speck extends Image {
	...
	public void reset( int index, float x, float y, int type ) {
		...
		case RED_LIGHT:
			tint(Xml.getColor("tangerine_yellow"));
		case LIGHT:
			angle = Random.Float( 360 );
			angularSpeed = 90;
			lifespan = 1f;
		...
		case TOXIC:
			hardlight( Xml.getColor("screamin_green") );
			angularSpeed = 30;
			angle = Random.Float( 360 );
		...
		case CORROSION:
			hardlight( Xml.getColor("dark_grey") );
			angularSpeed = 30;
			angle = Random.Float( 360 );
			lifespan = Random.Float( 1f, 3f );
			break;
		...
		case PARALYSIS:
			hardlight( Xml.getColor("laser_lemon") );
			angularSpeed = -30;
			angle = Random.Float( 360 );
			lifespan = Random.Float( 1f, 3f );
			break;
		...
		case CONFUSION:
			hardlight( Random.Int( Xml.getColor("black") ) | Xml.getColor("navy") );
		...
		case STORM:
			hardlight( Xml.getColor("morning_glory") );
			angularSpeed = Random.Float( -20, +20 );
			angle = Random.Float( 360 );
			lifespan = Random.Float( 1f, 3f );
			break;
		...
		case INFERNO:
			hardlight( Xml.getColor("pumpkin") );
		...
		case BLIZZARD:
			hardlight( Xml.getColor("pure_white") );
			angularSpeed = Random.Float( 200, 300 ) * (Random.Int(2) == 0 ? -1 : 1);
			angle = Random.Float( 360 );
			lifespan = Random.Float( 1f, 3f );
			break;
		...
		case SMOKE:
			hardlight( Xml.getColor("pure_black") );
			angularSpeed = 30;
			angle = Random.Float( 360 );
	...
	public void update() {
		...
		if (left <= 0) {
			...
			case CORROSION:
				hardlight( ColorMath.interpolate( Xml.getColor("dark_grey"), Xml.getColor("dark_orange") , p ));
			case STENCH:
			case SMOKE:

...
import com.ultimatepixel.ultimatepixeldungeon.effects.TargetedCell;
...
import com.ultimatepixel.ultimatepixeldungeon.sprites.LarvaSprite;
import com.ultimatepixel.ultimatepixeldungeon.sprites.YogSprite;
		...
		if (phase == 0){
			...
			if (abilityCooldown <= 0){
				...
				for (int i : targetedCells){
					Ballistica b = new Ballistica(pos, i, Ballistica.WONT_STOP);
					for (int p : b.path){
						sprite.parent.add(new TargetedCell(p, Xml.getColor("red")));
						affectedCells.add(p);
					}
				}

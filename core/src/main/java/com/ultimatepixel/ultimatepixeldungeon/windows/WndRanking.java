...
import com.ultimatepixel.ultimatepixeldungeon.ui.BadgesGrid;
import com.ultimatepixel.ultimatepixeldungeon.ui.BadgesList;
...
import com.ultimatepixel.ultimatepixeldungeon.ui.ItemSlot;
...
import com.ultimatepixel.ultimatepixeldungeon.ui.TalentButton;
import com.ultimatepixel.ultimatepixeldungeon.ui.TalentsPane;
		...
		public StatsTab() {
			...
			IconButton scoreInfo = new IconButton(Icons.get(Icons.INFO)){
				...
				protected void onClick() {
					super.onClick();
					UltimatePixelDungeon.scene().addToFront(new WndScoreBreakdown());
				}
			};
			...
			if (Dungeon.seed != -1 && !Dungeon.daily &&
				...
				RedButton btnSeed = new RedButton(Messages.get(this, "copy_seed")){
					...
					protected void onClick() {
						super.onClick();
						UltimatePixelDungeon.scene().addToFront(new WndOptions(new Image(icon),
							...
							protected void onSelect(int index) {
								...
								if (index == 0){
									UPDSettings.customSeed(DungeonSeed.convertToCode(Dungeon.seed));
				...
				if (DungeonSeed.convertFromText(UPDSettings.customSeed()) == Dungeon.seed){
					icon.hardlight(1f, 1.5f, 0.67f);
				}
				btnSeed.icon(icon);
		...
		public ChallengesTab(){
			...
			for (int i=0; i < Challenges.NAME_IDS.length; i++) {
				...
				IconButton info = new IconButton(Icons.get(Icons.INFO)){
					...
					protected void onClick() {
						super.onClick();
						UltimatePixelDungeon.scene().add(

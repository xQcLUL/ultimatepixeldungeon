...
import com.ultimatepixel.ultimatepixeldungeon.scenes.IntroScene;
	...
	public WndStartGame(final int slot){
		...
		if (DeviceCompat.isDebug() || Badges.isUnlocked(Badges.Badge.VICTORY)){
			IconButton challengeButton = new IconButton(
					Icons.get( UPDSettings.challenges() > 0 ? Icons.CHALLENGE_ON :Icons.CHALLENGE_OFF)){
				@Override
				protected void onClick() {
					UltimatePixelDungeon.scene().addToFront(new WndChallenges(UPDSettings.challenges(), true) {
						public void onBackPressed() {
							super.onBackPressed();
							if (parent != null) {
								icon(Icons.get(UPDSettings.challenges() > 0 ?
	...
	private class HeroPane extends Component {
		...
		protected void createChildren() {
			...
			heroItem = new IconButton(){
				...
				protected void onClick() {
					if (cl == null) return;
					UltimatePixelDungeon.scene().addToFront(new WndMessage(Messages.get(cl, cl.name() + "_desc_item")));
				}
			};
			...
			heroLoadout = new IconButton(){
				...
				protected void onClick() {
					if (cl == null) return;
					UltimatePixelDungeon.scene().addToFront(new WndMessage(Messages.get(cl, cl.name() + "_desc_loadout")));
				}
			};
			...
			heroMisc = new IconButton(){
				...
				protected void onClick() {
					if (cl == null) return;
					UltimatePixelDungeon.scene().addToFront(new WndMessage(Messages.get(cl, cl.name() + "_desc_misc")));
				}
			};
			...
			heroSubclass = new IconButton(new ItemSprite(ItemSpriteSheet.MASTERY, null)){
				...
				protected void onClick() {
					...
					UltimatePixelDungeon.scene().addToFront(new WndMessage(msg));
				}
			};

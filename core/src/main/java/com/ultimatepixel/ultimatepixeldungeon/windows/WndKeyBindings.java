...
import com.ultimatepixel.ultimatepixeldungeon.ui.ScrollPane;
...
public class WndKeyBindings extends Window {
	...
	public WndKeyBindings(Boolean controller) {
		...
		LinkedHashMap<Integer, GameAction> defaults = controller ? UPDAction.getControllerDefaults() : UPDAction.getDefaults();
		...
		RedButton btnDefaults = new RedButton(Messages.get(this, "default"), 9){
			...
			protected void onClick() {
				changedBindings = controller ? UPDAction.getControllerDefaults() : UPDAction.getDefaults();
				for (BindingItem i : listItems){
					int key1 = 0;
					int key2 = 0;
					int key3 = 0;
	...
	private class BindingItem extends Component {
		...
		private final int CHANGED = TITLE_COLOR;
		private final int DEFAULT = Xml.getColor("pure_white");
		private final int UNBOUND = Xml.getColor("grey");
		private final int UNBOUND_CHANGED = Xml.getColor("trendy_green");
		...
		private boolean onClick( float x, float y ){
			if (inside(x, y)){
				...
				if (x >= this.x + 4*width()/5 - 1 && key2 != 0) {
					UltimatePixelDungeon.scene().addToFront( new WndChangeBinding(gameAction, this, 3, key3, key1, key2));

				//assigning second key
				} else if (x >= this.x + 3*width()/5 - 1 && key1 != 0) {
					UltimatePixelDungeon.scene().addToFront( new WndChangeBinding(gameAction, this, 2, key2, key1, key3));

				//assigning first key
				} else if (x >= this.x + 2*width()/5){
					UltimatePixelDungeon.scene().addToFront( new WndChangeBinding(gameAction, this, 1, key1, key2, key3));
			...
			btnUnbind = new RedButton(Messages.get(this, "unbind"), 9){
				...
				protected void onClick() {
					if (action == GameAction.LEFT_CLICK && listItem.key2 == 0 && listItem.key3 == 0){
						UltimatePixelDungeon.scene().addToFront(new WndMessage(Messages.get(WndChangeBinding.class, "cant_unbind")));
					} else {

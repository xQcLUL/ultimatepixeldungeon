...
public class WndSettings extends WndTabbed {
	...
	public void hide() {
		...
		UltimatePixelDungeon.seamlessResetScene(new Game.SceneChangeCallback() {
			@Override
		...
		protected void createChildren() {
			...
			chkFullscreen = new CheckBox( Messages.get(this, "fullscreen") ) {
				...
				protected void onClick() {
					super.onClick();
					UPDSettings.fullscreen(checked());
				}
			};
			if (DeviceCompat.supportsFullScreen()){
				chkFullscreen.checked(UPDSettings.fullscreen());
			} else {
			...
			if (DeviceCompat.isAndroid() && PixelScene.maxScreenZoom >= 2) {
				chkSaver = new CheckBox(Messages.get(this, "saver")) {
					...
					protected void onClick() {
						...
						if (checked()) {
							checked(!checked());
							UltimatePixelDungeon.scene().add(new WndOptions(Icons.get(Icons.DISPLAY),
								...
								protected void onSelect(int index) {
									if (index == 0) {
										checked(!checked());
										UPDSettings.powerSaver(checked());
									}
								}
							});
						} else {
				...
				chkSaver.checked( UPDSettings.powerSaver() );
				add( chkSaver );
			}

			if (DeviceCompat.isAndroid()) {
				Boolean landscape = UPDSettings.landscape();
				if (landscape == null){
				...
				btnOrientation = new RedButton(finalLandscape ?
					...
					@Override
					protected void onClick() {
						UPDSettings.landscape(!finalLandscape);
					}
				};
				add(btnOrientation);
			}
			...
			optBrightness = new OptionSlider(Messages.get(this, "brightness"),
				...
				protected void onChange() {
					UPDSettings.brightness(getSelectedValue());
				}
			};
			optBrightness.setSelectedValue(UPDSettings.brightness());
			add(optBrightness);
			...
			optVisGrid = new OptionSlider(Messages.get(this, "visual_grid"),
				...
				@Override
				protected void onChange() {
					UPDSettings.visualGrid(getSelectedValue());
				}
			};
			optVisGrid.setSelectedValue(UPDSettings.visualGrid());
			add(optVisGrid);
		...
		protected void createChildren() {
			...
			if (Math.min(wMin, hMin) >= 2*Game.density){
				optUIMode = new OptionSlider(
					...
					@Override
					protected void onChange() {
						UPDSettings.interfaceSize(getSelectedValue());
						UltimatePixelDungeon.seamlessResetScene();
					}
				};
				optUIMode.setSelectedValue(UPDSettings.interfaceSize());
				add(optUIMode);
			}
			...
			if ((int)Math.ceil(2* Game.density) < PixelScene.maxDefaultZoom) {
				optUIScale = new OptionSlider(Messages.get(this, "scale"),
					...
					@Override
					protected void onChange() {
						if (getSelectedValue() != UPDSettings.scale()) {
							UPDSettings.scale(getSelectedValue());
							UltimatePixelDungeon.seamlessResetScene();
						}
					}
				};
			...
			if (UPDSettings.interfaceSize() == 0) {
				btnToolbarSettings = new RedButton(Messages.get(this, "toolbar_settings"), 9){
					@Override
					protected void onClick() {
						UltimatePixelDungeon.scene().addToFront(new Window(){
...

							{
								...
								btnSplit = new RedButton(Messages.get(WndSettings.UITab.this, "split")) {
									...
									protected void onClick() {
										...
										UPDSettings.toolbarMode(Toolbar.Mode.SPLIT.name());
										Toolbar.updateLayout();
									}
								};
								if (UPDSettings.toolbarMode().equals(Toolbar.Mode.SPLIT.name())) {
								...
								btnGrouped = new RedButton(Messages.get(WndSettings.UITab.this, "group")) {
									...
									protected void onClick() {
										...
										textColor(TITLE_COLOR);
										btnCentered.textColor(WHITE);
										UPDSettings.toolbarMode(Toolbar.Mode.GROUP.name());
										Toolbar.updateLayout();
									}
								};
								if (UPDSettings.toolbarMode().equals(Toolbar.Mode.GROUP.name())) {
								...
								btnCentered = new RedButton(Messages.get(WndSettings.UITab.this, "center")) {
									...
									protected void onClick() {
										...
										btnGrouped.textColor(WHITE);
										textColor(TITLE_COLOR);
										UPDSettings.toolbarMode(Toolbar.Mode.CENTER.name());
										Toolbar.updateLayout();
									}
								};
								if (UPDSettings.toolbarMode().equals(Toolbar.Mode.CENTER.name())) {
								...
								chkQuickSwapper = new CheckBox(Messages.get(WndSettings.UITab.this, "quickslot_swapper")) {
									...
									protected void onClick() {
										super.onClick();
										UPDSettings.quickSwapper(checked());
										Toolbar.updateLayout();
									}
								};
								chkQuickSwapper.checked(UPDSettings.quickSwapper());
								add(chkQuickSwapper);

								swapperDesc = PixelScene.renderTextBlock(Messages.get(WndSettings.UITab.this, "swapper_desc"), 5);
								swapperDesc.hardlight(Xml.getColor("grey"));
								add(swapperDesc);
								...
								chkFlipToolbar = new CheckBox(Messages.get(WndSettings.UITab.this, "flip_toolbar")) {
									...
									protected void onClick() {
										super.onClick();
										UPDSettings.flipToolbar(checked());
										Toolbar.updateLayout();
									}
								};
								chkFlipToolbar.checked(UPDSettings.flipToolbar());
								add(chkFlipToolbar);
								...
								chkFlipTags = new CheckBox(Messages.get(WndSettings.UITab.this, "flip_indicators")){
									...
									protected void onClick() {
										super.onClick();
										UPDSettings.flipTags(checked());
										GameScene.layoutTags();
									}
								};
								chkFlipTags.checked(UPDSettings.flipTags());
								add(chkFlipTags);
			...
			chkFont = new CheckBox(Messages.get(this, "system_font")){
				...
				protected void onClick() {
					...
					UltimatePixelDungeon.seamlessResetScene(new Game.SceneChangeCallback() {
						@Override
						public void beforeCreate() {
							UPDSettings.systemFont(checked());
						}
			...
			chkFont.checked(UPDSettings.systemFont());
			add(chkFont);
		}
		...
		protected void createChildren() {
			...
			if (DeviceCompat.hasHardKeyboard()){
				...
				btnKeyBindings = new RedButton(Messages.get(this, "key_bindings")){
					...
					protected void onClick() {
						super.onClick();
						UltimatePixelDungeon.scene().addToFront(new WndKeyBindings(false));
					}
				};
			...
			if (ControllerHandler.isControllerConnected()){
				btnControllerBindings = new RedButton(Messages.get(this, "controller_bindings")){
					...
					protected void onClick() {
						super.onClick();
						UltimatePixelDungeon.scene().addToFront(new WndKeyBindings(true));
					}
				};
			...
			optControlSens = new OptionSlider(
				...
				@Override
				protected void onChange() {
					UPDSettings.controllerPointerSensitivity(getSelectedValue());
				}
			};
			optControlSens.setSelectedValue(UPDSettings.controllerPointerSensitivity());
			add(optControlSens);
			...
			optHoldMoveSens = new OptionSlider(
				...
				@Override
				protected void onChange() {
					UPDSettings.movementHoldSensitivity(getSelectedValue());
				}
			};
			optHoldMoveSens.setSelectedValue(UPDSettings.movementHoldSensitivity());
			add(optHoldMoveSens);
		}
	...
	private static class AudioTab extends Component {
		...
		protected void createChildren() {
			...
			optMusic = new OptionSlider(Messages.get(this, "music_vol"), "0", "10", 0, 10) {
				@Override
				protected void onChange() {
					UPDSettings.musicVol(getSelectedValue());
				}
			};
			optMusic.setSelectedValue(UPDSettings.musicVol());
			add(optMusic);
			...
			chkMusicMute = new CheckBox(Messages.get(this, "music_mute")){
				...
				protected void onClick() {
					super.onClick();
					UPDSettings.music(!checked());
				}
			};
			chkMusicMute.checked(!UPDSettings.music());
			add(chkMusicMute);
			...
			optSFX = new OptionSlider(Messages.get(this, "sfx_vol"), "0", "10", 0, 10) {
				@Override
				protected void onChange() {
					UPDSettings.SFXVol(getSelectedValue());
					if (Random.Int(100) == 0){
			...
			optSFX.setSelectedValue(UPDSettings.SFXVol());
			add(optSFX);
			...
			chkMuteSFX = new CheckBox( Messages.get(this, "sfx_mute") ) {
				...
				protected void onClick() {
					super.onClick();
					UPDSettings.soundFx(!checked());
					Sample.INSTANCE.play( Assets.Sounds.CLICK );
				}
			};
			chkMuteSFX.checked(!UPDSettings.soundFx());
			add( chkMuteSFX );
			...
			if (DeviceCompat.isiOS() && Messages.lang() == Languages.ENGLISH){
				...
				chkIgnoreSilent = new CheckBox( Messages.get(this, "ignore_silent") ){
					...
					protected void onClick() {
						super.onClick();
						UPDSettings.ignoreSilentMode(checked());
					}
				};
				chkIgnoreSilent.checked(UPDSettings.ignoreSilentMode());
				add(chkIgnoreSilent);
			}
		}

...
public class WndBag extends WndTabbed {
	...
	protected void placeTitle( Bag bag, int width ){
		...
		if (Dungeon.energy == 0) {
			...
			amt.hardlight(Xml.getColor("summer_sky"));
			amt.measure();
			...
			protected void onClick() {
				if (lastBag != item && !lastBag.contains(item) && !item.isEquipped(Dungeon.hero)){
					...
					if(item instanceof Ring && ((Ring) item).isKnown()){
						...
						GameScene.show(new WndWeapon(WndBag.this, item));
					} else if(item instanceof Armor && item.isIdentified()){
						GameScene.show(new WndArmor(WndBag.this, item));
					} else if(item instanceof Wand && item.isIdentified()){
						GameScene.show(new WndWand(WndBag.this, item));
					} else {
	...
	public boolean onSignal(KeyEvent event) {
		if (event.pressed && KeyBindings.getActionForKey( event ) == UPDAction.INVENTORY) {
			hide();
			return true;
		} else {
	...
	private class BagTab extends IconTab {
		...
		public GameAction keyAction() {
			switch (index){
				case 1: default:
					return UPDAction.BAG_1;
				case 2:
					return UPDAction.BAG_2;
				case 3:
					return UPDAction.BAG_3;
				case 4:
					return UPDAction.BAG_4;
				case 5:
					return UPDAction.BAG_5;
			}
		}
